  ...
  search: procedure
    .input(
      z.object({
        ...
        orderBy: z.enum(["price_asc", "price_desc", "name_asc", "name_desc"]).optional().describe("The vehicle list ordena.")
      }),
    )
    ...
    .query(async ({ input }) => {
      try {
        ...
        let ordernation: Record<string, "asc" | "desc"> = {};

        if (orderBy) {
          switch (orderBy) {
            case "name_asc":
              ordernation = { make: "asc" }
              break
            case "name_desc":
              ordernation = { make: "desc" }
              break
            case "price_asc":
              ordernation = { hourly_rate_cents: "asc" }
              break
            case "price_desc":
              ordernation = { hourly_rate_cents: "desc" }
              break
            default:
              ordernation = { hourly_rate_cents: "asc" }
          }
